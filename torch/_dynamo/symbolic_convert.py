...
class InstructionTranslatorBase(
    ...
    def DELETE_GLOBAL(self, inst: Instruction) -> None:
        ...
        if name not in self.f_globals:
            if pending is None:
                # `symbolic_globals` is never pruned, so a sentinel for a name
                # that is absent from the real globals only means a STORE_GLOBAL
                # created it earlier in this trace. If the pending store is gone
                # too, an earlier `del` already cancelled it and the name is
                # unbound, as in `g = 1; del g; del g`.
                self.raise_name_error(name)
            # The delete cancels that pending store and there is nothing to
            # replay. Same mechanism as the cross-module handler.
            side_effects.discard_attr_mutation(variable, name, GlobalSource(name))
            return
...
class InliningInstructionTranslator(InstructionTranslatorBase):
    ...
    def DELETE_GLOBAL(self, inst: Instruction) -> None:
        ...
        pending = side_effects.store_attr_mutations.get(fglobals_vt, {}).get(name)
        if isinstance(pending, variables.DeletedVariable):
            # An earlier `del` in this trace already removed the name.
            self.raise_name_error(name)
        if isinstance(pending, TensorVariable):
            # See the root-frame handler: dropping `pending` destroys the last
            # reference to a store made earlier in this trace.
            self._maybe_emit_sync_dealloc(pending)
        if name not in self.f_globals:
            if pending is None:
                # Nothing in this trace bound the name either.
                self.raise_name_error(name)
            # STORE_GLOBAL created this name only during tracing, so the delete
            # cancels the pending store and there is nothing to replay against
